...
void read (students &a)
{
    ...
    getline(cin, a.name);
    cout<<endl;
    cout<<"Godinata v koqto se zapisan stidenta:";
    cin>>a.godina;
...
void print1(students a)
{
    ...
    cout<<"Godinata v koqto e zapisan stidenta:"<<a.godina<<endl;
...
void sortirane(vector<students> v, int n)
{
    for(int i=0; i<n; i++)
    {
        for(int j=0; j<n-i; j++)
        {
            if(v[j].nomerSpec> v[j+1].nomerSpec)
            {
                swap(v[j].nomerSpec, v[j+1].nomerSpec);
            }
            if(v[j].nomerSpec== v[j+1].nomerSpec)
            {
                for(int i=0; i<n; i++)
                {
                    for(int j=0; j<n-i; j++)
                    {
                        if(v[j].god> v[j+1].god)
                        {
                            swap(v[j].god, v[j+1].god);
                        }
                    }
                }
